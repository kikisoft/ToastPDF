...
HGLOBAL h_extra_env;
    ...
    if (prd->config.dwRunUser) {
		...
		if (prd->primary_token != NULL)
			if (!CreateEnvironmentBlock(&env, prd->primary_token, FALSE))
				env = NULL;
	}
#endif
	if (env == NULL)
...
BOOL start_redirect(REDATA * prd)
{
	...
	// #####
	// Retrieve the station ID and desktop for the user process using:
	// GetProcessWindowStation and GetDesktopThread (while retrieving primary token...)
